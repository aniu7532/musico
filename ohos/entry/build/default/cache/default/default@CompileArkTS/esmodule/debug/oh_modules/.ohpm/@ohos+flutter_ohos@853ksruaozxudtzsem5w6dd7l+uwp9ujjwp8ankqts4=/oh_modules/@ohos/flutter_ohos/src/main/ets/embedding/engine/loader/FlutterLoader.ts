...
export default class FlutterLoader {
    ...
    async startInitialization(context: common.Context) {
        ...
        this.initResult = new InitResult(`${context.filesDir}/`, `${context.cacheDir}/`, `${context.filesDir}`);
    ...
    ensureInitializationComplete(shellArgs: Array<string> | null) {
        ...
        shellArgs.push("--icu-native-lib-path="
        ...
        if (this.flutterApplicationInfo!.isDebugMode) {
            ...
            shellArgs.push("--" + ISOLATE_SNAPSHOT_DATA_KEY + "=" + this.flutterApplicationInfo!.isolateSnapshotData);
            shellArgs.push('--enable-checked-mode');
            shellArgs.push('--verbose-logging');
        }
        else {
            shellArgs.push("--" + AOT_SHARED_LIBRARY_NAME + "=" + this.flutterApplicationInfo!.aotSharedLibraryName);
            shellArgs.push("--"
        ...
        this.flutterNapi.init(this.context!, shellArgs, kernelPath, this.initResult!.appStoragePath, this.initResult!.engineCachesPath!, costTime);
...
class InitResult {
    ...
    dataDirPath: string;
    constructor(appStoragePath: string, engineCachesPath: string, dataDirPath: string) {
