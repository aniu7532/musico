...
export default class TextInputChannel {
    ...
    createEditingStateJSON(text: string, selectionStart: number, selectionEnd: number, composingStart: number, composingEnd: number): EditingState {
     ...
     */
    updateEditingState(inputClientId: number, text: string, selectionStart: number, selectionEnd: number, composingStart: number, composingEnd: number): void {
...
export class Configuration {
    ...
    constructor(obscureText: boolean, autocorrect: boolean, enableSuggestions: boolean, enableIMEPersonalizedLearning: boolean, enableDeltaModel: boolean, textCapitalization: TextCapitalization, inputType: InputType, inputAction: Number, actionLabel: String, autofill: boolean, contentListString: [
    ], fields: Configuration[]) {
...
export class InputType {
    ...
    static fromJson(json: Any): InputType {
        return new InputType(InputType.getTextInputTypeFromValue(json.name as string), json.signed as boolean, json.decimal as boolean);
    }
...
export class TextEditState {
    ...
    composingEnd: number;
    constructor(text: string, selectionStart: number, selectionEnd: number, composingStart: number, composingEnd: number) {
    ...
    static fromJson(textEditState: Any): TextEditState {
        if (textEditState.text != null && textEditState.text != undefined && textEditState.text != "") {
            return new TextEditState(textEditState.text, textEditState.selectionBase, textEditState.selectionExtent, textEditState.composingBase, textEditState.composingExtent);
        }
        else {
            return new TextEditState(textEditState.get('text'), textEditState.get('selectionBase'), textEditState.get('selectionExtent'), textEditState.get('composingBase'), textEditState.get('composingExtent'));
        }
    }
}
