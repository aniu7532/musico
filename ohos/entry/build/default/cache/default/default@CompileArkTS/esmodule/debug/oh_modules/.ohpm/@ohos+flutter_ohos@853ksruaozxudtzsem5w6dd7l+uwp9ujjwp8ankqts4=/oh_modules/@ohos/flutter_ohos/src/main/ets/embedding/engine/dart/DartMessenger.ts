...
export class DartMessenger implements BinaryMessenger, PlatformMessageHandler {
    ...
    makeBackgroundTaskQueue(options?: TaskQueueOptions): TaskQueue {
        let taskQueue: DartMessengerTaskQueue = this.taskQueueFactory.makeBackgroundTaskQueue(options ?? new TaskQueueOptions());
    ...
    dispatchMessageToQueue(handlerInfo: HandlerInfo, message: ArrayBuffer, replyId: number): void {
        let taskState: TaskState = new TaskState(handlerInfo.handler as any, message, ...handlerInfo.args);
...
class HandlerInfo {
    ...
    args: Object[];
    constructor(handler: BinaryMessageHandler | SendableBinaryMessageHandler, taskQueue: DartMessengerTaskQueue | null, ...args: Object[]) {
...
class ConcurrentTaskQueue implements DartMessengerTaskQueue {
    ...
    dispatch(taskState: TaskState, callback: Reply): void {
        let task: taskpool.Task = new taskpool.Task(handleMessageInBackground, taskState.handler, taskState.message, ...taskState.args);
...
async function handleMessageInBackground(handler: SendableBinaryMessageHandler, message: ArrayBuffer, ...args: Object[]): Promise<ArrayBuffer | null> {
    "use concurrent";
