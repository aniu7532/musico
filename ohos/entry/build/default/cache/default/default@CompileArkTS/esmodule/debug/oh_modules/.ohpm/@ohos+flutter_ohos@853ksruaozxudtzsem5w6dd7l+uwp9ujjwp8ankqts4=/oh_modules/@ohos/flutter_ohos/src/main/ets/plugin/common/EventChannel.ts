...
export default class EventChannel {
    ...
    setStreamHandler(handler: StreamHandler): void {
        ...
        if (this.taskQueue != null) {
            this.messenger.setMessageHandler(this.name, handler == null ? null : new IncomingStreamRequestHandler(handler, this.name, this.codec, this.messenger), this.taskQueue);
        }
        else {
            this.messenger.setMessageHandler(this.name, handler == null ? null : new IncomingStreamRequestHandler(handler, this.name, this.codec, this.messenger));
        }
    }
}
/**
...
class EventSinkImplementation implements EventSink {
    ...
    error(errorCode: string, errorMessage: string, errorDetails: Any) {
        ...
        this.messenger.send(this.name, this.codec.encodeErrorEnvelope(errorCode, errorMessage, errorDetails));
    }
    endOfStream(): void {
        if (this.hasEnded || this.activeSink.get() != this) {
            return;
