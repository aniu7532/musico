...
export default class FlutterNapi {
    ...
    init(context: common.Context, args: Array<string>, bundlePath: string, appStoragePath: string, engineCachesPath: string, initTimeMillis: number) {
        ...
        let code: number | null = flutter.nativeInit(context, args, bundlePath, appStoragePath, engineCachesPath, initTimeMillis, deviceInfo.productModel);
    ...
    runBundleAndSnapshotFromLibrary(bundlePath: string, entrypointFunctionName: string | undefined, pathToEntrypointFunction: string | undefined, assetManager: resourceManager.ResourceManager, entrypointArgs: Array<string>) {
        if (!FlutterNapi.hasInit) {
    ...
    invokePlatformMessageResponseCallback(responseId: number, message: ArrayBuffer, position: number) {
        if (this.isAttached()) {
            flutter.nativeInvokePlatformMessageResponseCallback(this.nativeShellHolderId!, responseId, message, position);
        }
        else {
            Log.w(TAG, "Tried to send a platform message response, but FlutterNapi was detached from native C++. Could not send. Response ID: "
                + responseId);
    ...
    xComponentDisPatchMouseWheel(xcomponentId: string, eventType: string, event: PanGestureEvent) {
        ...
        flutter.nativeXComponentDispatchMouseWheel(this.nativeShellHolderId!!, xcomponentId, eventType, vaildFinger?.id, vaildFinger?.localX, vaildFinger?.localY, event.offsetY, event.timestamp);
    }
