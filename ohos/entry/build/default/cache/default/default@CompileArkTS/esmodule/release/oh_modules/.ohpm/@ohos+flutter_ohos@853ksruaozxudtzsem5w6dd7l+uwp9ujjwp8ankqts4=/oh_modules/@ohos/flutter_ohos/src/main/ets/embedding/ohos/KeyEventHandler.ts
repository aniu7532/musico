...
export class KeyEventHandler {
    ...
    constructor(t41?: TextInputPlugin) {
        this.textInputPlugin = t41;
        this.initCharMap();
        this.initShiftMap();
    }
    private initCharMap() {
    ...
    getCharByEvent(r41: KeyEvent): string {
        let s41 = r41.keyCode;
        if (this.isShiftMode) {
            return this.shiftMap.hasKey(s41) ? this.shiftMap.get(s41) : '';
        }
        else {
            return this.charMap.hasKey(s41) ? this.charMap.get(s41) : '';
        }
    }
    handleKeyEvent(q41: KeyEvent) {
        Log.i(TAG, JSON.stringify({
            "name": "handleKeyEvent",
            "event": q41
        }));
        if (q41.type == KeyType.Up) {
            if (this.charMap.hasKey(q41.keyCode)) {
                this.textInputPlugin?.getEditingState().handleInsertTextEvent(this.getCharByEvent(q41));
            }
            if (q41.keyCode == KeyCode.KEYCODE_DEL) {
                this.textInputPlugin?.getEditingState().handleDeleteEvent(false, 0);
            }
        }
        this.isShiftMode = q41.keyCode == KeyCode.KEYCODE_SHIFT_LEFT;
    }
}
