...
export default class PlatformViewsChannel {
    ...
    create(call: MethodCall, result: MethodResult): void {
        ...
        try {
            if (usesPlatformViewLayer) {
                const request: PlatformViewCreationRequest = new PlatformViewCreationRequest(createArgs.get("id"), createArgs.get("viewType"), 0, 0, 0, 0, direction, additionalParams, RequestedDisplayMode.HYBRID_ONLY);
                ...
                const displayMode: RequestedDisplayMode = hybridFallback ? RequestedDisplayMode.TEXTURE_WITH_HYBRID_FALLBACK
    ...
    resize(call: MethodCall, result: MethodResult): void {
        ...
        const resizeRequest: PlatformViewResizeRequest = new PlatformViewResizeRequest(resizeArgs.get("id"), resizeArgs.get("width"), resizeArgs.get("height"));
        try {
    ...
    offset(call: MethodCall, result: MethodResult): void {
        ...
        try {
            this.handler?.offset(offsetArgs.get("id"), offsetArgs.get("top"), offsetArgs.get("left"));
            result.success(null);
        }
        catch (err) {
    ...
    touch(call: MethodCall, result: MethodResult): void {
        ...
        let index = 0;
        const touch: PlatformViewTouch = new PlatformViewTouch(args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index++], args[index]);
        try {
...
export class PlatformViewCreationRequest {
    ...
    constructor(viewId: number, viewType: string, logicalTop: number, logicalLeft: number, logicalWidth: number, logicalHeight: number, direction: Direction, params: ByteBuffer, displayMode?: RequestedDisplayMode) {
        this.viewId = viewId;
...
export class PlatformViewTouch {
    ...
    constructor(viewId: number, downTime: number, eventTime: number, action: number, pointerCount: number, rawPointerPropertiesList: Any, rawPointerCoords: Any, metaState: number, buttonState: number, xPrecision: number, yPrecision: number, deviceId: number, edgeFlags: number, source: number, flags: number, motionEventId: number) {
        this.viewId = viewId;
