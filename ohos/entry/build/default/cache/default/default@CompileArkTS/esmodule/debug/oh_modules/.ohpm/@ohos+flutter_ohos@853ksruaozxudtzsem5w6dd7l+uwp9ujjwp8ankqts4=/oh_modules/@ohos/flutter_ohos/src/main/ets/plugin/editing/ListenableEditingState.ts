...
export class ListenableEditingState {
    ...
    replace(start: number, end: number, tb: String, tbStart: number, tbEnd: number): void {
        ...
        this.mBatchTextEditingDeltas.add(new TextEditingDelta(this.mStringCache.toString(), placeIndex + tbEnd, placeIndex + tbEnd, this.getComposingStart(), this.getComposingEnd(), start, end + tbStart, tb.toString()));
    }
    ...
    notifyListener(listener: EditingStateWatcher, textChanged: boolean, selectionChanged: boolean, composingChanged: boolean): void {
    ...
    handleDeleteEvent(leftOrRight: boolean, length: number): void {
        ...
        if (leftOrRight == false) {
            ...
            if (start == end) {
                unicodeStart = FlutterTextUtils.getOffsetBefore(this.mStringCache, start);
            }
            ...
            let unicodeEnd = end;
            if (start == end) {
                unicodeEnd = FlutterTextUtils.getOffsetAfter(this.mStringCache, start);
            }
